...
class DetectionTab:
    ...
    # Replace the toggle_ml_detection method in ui/detection_tab.py
    ...
    def toggle_ml_detection(self, event=None):
        ...
        if new_value == "On":
            # Check if we need to initialize the ML detector
            if self.app.ml_detector is None:
                try:
                    ...
                    message_label = Label(loading_window, text="Loading ML model...", font=("Arial", 12))
                    ...
                    def init_with_timeout():
                        try:
                            ...
                            self.app.log_event(f"ML initialization error: {str(e)}")
                    ...
                    # Wait with timeout and update UI
                    start_time = time.time()
                    while not init_complete.is_set() and (time.time() - start_time) < 30:
                        message_label.config(text=f"Loading ML model... {int(time.time() - start_time)}s")
                        loading_window.update()
                        time.sleep(0.1)
                    ...
                    if not init_complete.is_set():
                        messagebox.showerror("ML Error", "Model initialization timed out")
                        self.ml_var.set("Off")
                        self.app.log_event("ML initialization timed out")
                        return

                    if not init_success[0]:
                        messagebox.showerror("ML Error", "Failed to initialize ML detector")
                        self.ml_var.set("Off")
                        return
            ...
            self.app.log_event("ML detection enabled")

            # Reset detection to use the new setting
            if self.app.running:
                # Reset frame tracking variables to force redetection
                self.prev_frame = None
                self.frame_count = 0
                self.last_detections = []
            ...
            # Reset frame tracking to force traditional detection
            if self.app.running:
                self.prev_frame = None
                self.frame_count = 0
                self.last_detections = []

    # Add this helper method to the DetectionTab class
    ...
    def switch_video_source(self, source):
        ...
        try:
            ...
            if not self.app.video_capture.isOpened():
                raise Exception(f"Could not open video source: {source}")

            # Get frame dimensions
            _, first_frame = self.app.video_capture.read()
            ...
            self.video_canvas.config(width=self.app.image_width, height=self.app.image_height)
            self.app.log_event(f"Switched to video source: {source} ({self.app.image_width}x{self.app.image_height})")

            # Update reference image based on video source
            if source in self.app.video_reference_map:
                new_ref_image = self.app.video_reference_map[source]
                self.app.current_reference_image = new_ref_image

                # Load the corresponding parking positions
                self.app.load_parking_positions(new_ref_image)

                # The scaling is now done in load_parking_positions

            # Reset parameters for new source
            ...
            messagebox.showerror("Error", f"Failed to switch video source: {source} - {str(e)}")
